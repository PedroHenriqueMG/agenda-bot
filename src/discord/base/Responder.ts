...
type ResolveParams<Path, Parsed> = Parsed extends {
...
> {
	...
	private static getResponderType(
	...
	public static async onInteraction(
		...
		const router =
			Responder.routers.get(responderType) ??
			Responder.findRouter(responderType)
		if (!router) {
			Responder.options.onNotFound?.(interaction)
			return
		}

		const handler = Responder.findHandler(
			router,
			responderType,
	...
	private static findHandler(
		router: ResponderRouter | undefined,
		type: ResponderType,
		customId: string
	): MatchedRoute<ResponderDataGeneric> | null {
...
type CheckRoute<R> = R extends `/${string}`
	? never
	: R extends `${string}/`
		? never
		: R extends `:${string}`
...
type ExtractParam<Seg> = Seg extends `:${infer Param}`
	? Param
...
type GetParams<Route> = Route extends `${infer Seg}/${infer Rest}`
