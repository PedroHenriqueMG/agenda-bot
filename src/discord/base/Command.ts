...
type CommandProps<N, D, T> = T extends ApplicationCommandType.ChatInput
	? ChatInputApplicationCommandData & {
			name: SlashName<N>
...
> {
	...
	constructor(public readonly data: CommandData<N, T, D>) {
		...
		Command.handlers.set(data.name, {
			run: data.run,
			autocomplete: "autocomplete" in data ? data.autocomplete : undefined,
		})
	}
	public static async register(options: CommandRegisterOptions) {
		...
		if (guilds?.size) {
			const [globalCommands, guildCommads] = Command.commands.partition(
				c => c.global === true
			)
			...
			for (const guild of guilds.values()) {
				await guild.commands
					.set(Array.from(guildCommads.values()))
					.then(({ size }) =>
						addMessage(
